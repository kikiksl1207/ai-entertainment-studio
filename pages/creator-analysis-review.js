(function initWriterAnalysis() {
  ...
  const generationCopy = {
    ko: {
      ...
      restoreMissing: "이 작품의 최신 원고에는 완료된 AI 분석이 없습니다.", restoreFailed: "기존 분석을 불러오지 못했습니다. 다시 시도해 주세요.",
    ...
    en: {
      ...
      restoreMissing: "The latest manuscript for this work has no completed AI analysis.", restoreFailed: "Could not load the saved analysis. Try again.",
    ...
    ja: {
      ...
      restoreMissing: "この作品の最新原稿には完了したAI分析がありません。", restoreFailed: "保存済みの分析を読み込めませんでした。もう一度お試しください。",
    ...
    "zh-Hans": {
      ...
      restoreMissing: "本作品的最新稿件没有已完成的 AI 分析。", restoreFailed: "无法加载已保存的分析，请重试。",
    ...
    "zh-Hant": {
      ...
      restoreMissing: "本作品的最新稿件沒有已完成的 AI 分析。", restoreFailed: "無法載入已儲存的分析，請重試。",
  ...
  async function restoreExisting() {
    ...
    try {
      ...
      const latest = analysisPage.items.find(item => item?.kind === "semantic_extraction_v1" && item.status === "completed" &&
        item.semanticCompleted === true && item.progress?.coverageComplete === true && item.sourceLocale === source.locale &&
        item.sourceContentHash === source.contentHash && uuid.test(item.id || ""));
      if (!latest) { restore.state.textContent = gt("restoreMissing"); return; }
      receive({ id: source.id, workId: source.workId, sourceLocale: source.locale, version: source.version,
        contentHash: source.contentHash, identity: scope.identity }, { existingAnalysisId: latest.id });
