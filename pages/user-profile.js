...
const FOLLOW_PROFILE_FIXTURE_DEFAULT_HANDLE = "relationship-preview";
...
function normalizeFixtureHandle(value) {
  const raw = String(value || FOLLOW_PROFILE_FIXTURE_DEFAULT_HANDLE)
    .trim()
    .toLowerCase()
    .replace(/^@/, "");
...
function buildFollowProfileFixture(params) {
  ...
  const handle = isSelfFixture ? "relationship-preview-user" : requestedHandle;
  const displayName = isSelfFixture ? "관계 미리보기 계정" : "팔로워 미리보기 프로필";

  return {
    fixture: {
      source: "followfixture",
      readOnly: true,
      fixtureStatus: "profile_fixture_preview_only",
    },
    user: {
      id: isSelfFixture ? "profile-preview-user" : "profile-preview-target",
      displayName,
      publicHandle: handle,
    ...
    stats: {
      followerCount: FOLLOW_LIST_FIXTURE.followers.length,
      followingCount: FOLLOW_LIST_FIXTURE.following.length,
      postCount: 0,
    },
    viewer: {
      isSelf: isSelfFixture,
      canEditProfile: false,
      isAuthenticated: isSelfFixture,
      isFollowing: false,
      canFollow: !isSelfFixture,
      canUnfollow: false,
    },
  };
}

function renderFollowProfileFixturePosts() {
  const section = document.getElementById("userProfilePostsSection");
  const list = document.getElementById("userProfilePostList");
  const empty = document.getElementById("userProfilePostsEmpty");
  const loadMore = document.getElementById("userProfileLoadMore");
  if (section) {
...
async function initUserProfilePage() {
  ...
  if (userProfileUsesFollowFixture(params)) {
...
function renderUserProfileCard(data) {
  ...
  if (handleEl) {
    ...
    const isPreviewHandle = handle.startsWith("relationship-preview");
    const isAutoGenerated = !isPreviewHandle && (/^user-[a-f0-9]{16,}$/i.test(handle) || handle.length > 24);
  ...
  const isReadOnlyFixture = !!data.fixture?.readOnly;
  // #1055 — 사용자 차단 진입점 (남의 프로필에만 노출)
  const blockBtn = document.getElementById("userProfileBlockBtn");
  if (isSelf) {
    if (editBtn)   { editBtn.hidden = isReadOnlyFixture;  editBtn.style.display = isReadOnlyFixture ? "none" : ""; }
...
function bindFollowListStatClick() {
  const statButtons = document.querySelectorAll("[data-follow-open]");
  if (statButtons.length) {
    statButtons.forEach(btn => {
      if (btn._followOpenBound) return;
      btn._followOpenBound = true;
      btn.addEventListener("click", () => openFollowListModal(btn.dataset.followOpen || "followers"));
    });
    return;
  }
...
function renderFollowListItem(item, type) {
  ...
  let actionsHtml = "";
  if ((_followListIsSelf || followListUsesFixture()) && type === "followers" && userId) {
    actionsHtml = `
