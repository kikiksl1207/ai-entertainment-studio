...
const WALLET_ADJUSTMENT_PLACEHOLDER_NOTES = new Set(['Î∞±Ïä§?åÏù¥ÏßÄ ?¥ÏòÅ Ï≤òÎ¶¨']);
...
export class AdminService {
  ...
  async getBackstageArtistKnowledgeUrlAuditEvents(query: AuditQuery) {
    ...
    if (action && !supportedActions.includes(action)) {
      this.throwArtistKnowledgeBadRequest(
        ...
        '?Î®?¶∫ URL Â™õÎ®ØÍ∂???™ÎÄ??Íæ™ÍΩ£???Î∫§Ïî§??‰∫åÏá±ÍΩ??',
        { supportedActions },
      );
    }
  ...
  async approveBackstageArtistKnowledgeUrl(
    ...
    if (existing.status === 'archived') {
      throw new ConflictException({
        ...
        message: 'Î≥¥Í????êÎ£å URL?Ä ?πÏù∏?????ÜÏäµ?àÎã§.',
  ...
  async rejectBackstageArtistKnowledgeUrl(
    ...
    if (existing.status === 'archived') {
      throw new ConflictException({
        code: 'ARTIST_KNOWLEDGE_URL_ARCHIVED',
        message: 'Î≥¥Í????êÎ£å URL?Ä Î∞òÎ†§?????ÜÏäµ?àÎã§.',
        messageKey: 'artistKnowledgeUrl.error.archived',
      });
    }
  ...
  private throwArtistKnowledgeNotFound(): never {
    throw new NotFoundException({
      ...
      message: '?êÎ£å URL??Ï∞æÏùÑ ???ÜÏäµ?àÎã§.',
  ...
  private aiContentNextAction(key: string) {
    const labels: Record<string, string> = {
      public_profile: '?ÑÌã∞?§Ìä∏ Í≥µÍ∞ú ?åÍ∞ú/?úÍ∑∏?ºÏù∏??Î≥¥Í∞ï?©Îãà??',
      visual_profile: 'ÎπÑÏ£º???§Ïõå???êÎäî ?§Ì????∏Ìä∏Î•?Î≥¥Í∞ï?©Îãà??',
      content_profile: 'ÏΩòÌÖêÏ∏??§Ïï§Îß§ÎÑà ?êÎäî ?¥ÏòÅ ?∏Ìä∏Î•?Î≥¥Í∞ï?©Îãà??',
      cover_asset: 'Ïª§Î≤Ñ/?àÏñ¥Î°??¨Î°Ø ?¥Î?ÏßÄÎ•??∞Í≤∞?©Îãà??',
      thumbnail_asset: '?∏ÎÑ§???ÑÎ°ú???¨Î°Ø ?¥Î?ÏßÄÎ•??∞Í≤∞?©Îãà??',
      gallery_assets: 'Í∞§Îü¨Î¶??¨Î°Ø ?¥Î?ÏßÄÎ•?1???¥ÏÉÅ ?∞Í≤∞?©Îãà??',
      shortforms: 'Í≥µÍ∞ú ?êÎäî Ï§ÄÎπ?Ï§??èÌèº??1Í∞??¥ÏÉÅ ?±Î°ù?©Îãà??',
      chat_persona: 'Ï∫êÎ¶≠?∞Ï±ó??persona Ï¥àÏïà??Ï§ÄÎπÑÌï©?àÎã§.',
    };

    return {
      key,
      label: labels[key] ?? '?¥ÏòÅ?êÍ? ?ÅÌÉúÎ•??ïÏù∏?©Îãà??',
    };
  }
