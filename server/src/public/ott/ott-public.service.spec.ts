...
const registry = JSON.stringify([{ slug: 'author-cut', workId, manifestId, rightsContractVersionId: rightsId,
...
function fixture() {
  const prisma = {
    ...
    contentRightsContractVersion: { findFirst: jest.fn().mockResolvedValue({ media: ['ott_streaming'], startsAt: new Date(0), endsAt: null, effectiveFrom: new Date(0) }) },
  };
...
describe('OttPublicService', () => {
  ...
  it('omits a release when rights, pins, or confirmed media no longer pass', async () => {
    ...
    prisma.contentRightsContractVersion.findFirst.mockResolvedValue(null);
