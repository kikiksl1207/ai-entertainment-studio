...
- `PATCH /api/v1/me/profile` accepts `displayName`, `bio`, and `avatarAssetId`. `displayName` is 2-20 characters and is server-limited to one change every 30 days through `user_profiles.nickname_changed_at`; active cooldown returns `429`.
