...
- Use `POST /api/v1/lumina-feed/posts/:postId/share` to request the public share contract for a public published post, including another user's post. It returns `share.publicPath`, `share.webShare`, and `share.countStrategy: "not_mutated_by_share_contract"`. It does not require author ownership and does not create a repost, feed row, share ledger, wallet, Lumina, settlement, payout, order, or paid-like mutation.
