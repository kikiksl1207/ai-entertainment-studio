...
- Provider email templates use the shared Lumina Stage responsive brand layout for email verification and password reset. The saved account locale selects Korean, English, Japanese, Simplified Chinese, or Traditional Chinese copy, with Korean as the fallback. Body copy avoids user-facing provider/API/token wording, while the action link still carries the opaque action token in the URL contract.
- Email expiry copy uses a locale-formatted, human-readable timestamp generated by the backend and always labels the `KST` timezone.
...
- Response rows expose only operationally safe fields: `id`, `purpose`, derived `status`, `statusKey`, `createdAt`, `expiresAt`, `consumedAt`, masked `target.emailMasked`, masked stable `target.userRef`, `target.userStatus`, `target.emailVerified`, and `target.deleted`. Raw internal user ids are accepted only as admin filters and are not echoed in the projection.
