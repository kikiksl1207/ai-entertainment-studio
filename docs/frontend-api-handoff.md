...
- `GET /rewards/activation-progress` returns the signed-in user's reward progress and cap usage. Use this for My Page/Charge Station reward cards before adding claim buttons. Planned milestones are not claimable until the backend exposes a live grant endpoint.
