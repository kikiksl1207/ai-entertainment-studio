...
- `GET /me/trust` returns the current user's trust/role gate state for abuse-sensitive actions. Use this to decide whether to show "identity verification required" messaging before referral rewards, paid support, fan letters, and future creator settlement surfaces. Identity verification is a fail-closed NICE-first skeleton until the real adapter is connected. The response includes `accountState.ageGate`, `accountState.cleanMode`, `settlement.identityVerification`, `settlement.payoutAccount`, and `settlement.payoutException`. Signup remains open before identity verification; minor clean mode is enforced only when the backend reports a verified provider birth-date minor.
