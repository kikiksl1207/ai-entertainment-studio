...
Run `npm run build`, the story production tests, `npm run qa:story-production-release`, and `npm run qa:story-player-read-apis`. QA commands emit only a run ID, public path, status, and boolean checks. Applying migration `0048_story_production_backend` and exercising a real published story require a configured staging database and an approved non-secret test identity; no credential material belongs in reports.
