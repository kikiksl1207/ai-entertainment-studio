...
| Auth action token audit | `GET /auth/action-tokens` | `audit:read` | Read-only email verification/password reset token trace. Returns masked target email and derived token state only; raw token, token hash, mail body, provider secret, and env values are never returned. Delivery status/provider are reported as not recorded until delivery persistence exists. |
