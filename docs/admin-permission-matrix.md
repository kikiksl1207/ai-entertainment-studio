...
| Auth action token audit | `GET /auth/action-tokens` | `audit:read` | Read-only email verification/password reset token trace. Supports token state plus delivery status/provider filters. Returns masked target email, masked stable user ref, derived token state, and persisted delivery status/provider timestamps only; raw user id, raw token, token hash, mail body, provider raw response, provider secret, and env values are never returned. |
